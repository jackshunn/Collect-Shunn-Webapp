...
export default function FocusedList(props) {
    ...
    function handleChange(event){
        setList(prev => {
            let newList = {...prev};
            newList.items[0].text = event.target.value;
            return newList;
        })
    }
    ...
    <div className="py-9 rounded-lg  bg-customColor-lightBlue flex flex-col flex-1" onClick={handleInsideClick}>
        ...
        <div className="flex">
            ...
            <div className="flex-1 m-5 rounded-xl h-11 bg-customColor-darkBlue flex">
                <input type="text" className="flex-1 bg-transparent text-white text-xl mx-3" value={list.items[0].text} onChange={handleChange}></input>
